        ...
        let mut est_w = if pct_w_skip || pct_w_indefinite {
            ...
            if let Some(t) = &ch.text {
                if ch.taffy_mode {
                    ...
                    super::super::layout::measure_text_width_full(t, ch.font_size, ch.effective_weight(), ch.italic, &ch.font_family, ch.letter_spacing)
                }
...
fn intrinsic_content_width(bx: &LayoutBox) -> f32 {
    ...
    if let Some(t) = &bx.text {
        return super::super::layout::measure_text_width_full(t, bx.font_size, bx.effective_weight(), bx.italic, &bx.font_family, bx.letter_spacing) + chrome;
    }
